...
// 20 MS per message
// 5 messages = 100MS total processing time
#define CAN_UPDATE_MS (50)
...
#define CAN_FLAG_HANDLER_QUEUED (0x01)
#define CAN_FLAG_HANDLER_WAITING (0x02)
...
void handle_can_rx_isr() {
    incoming_packet_length = mcp.parsePacket();
    if (mcp.packetRtr()) {
        ...
        if (!rx_buffer.full()) {
            ...
            can_flags |= CAN_FLAG_HANDLER_WAITING;
        }
    }
}

void poll_can_rx(void* ctx) {
...
void can_update_cb(void* ctx) {
    poll_can_rx(NULL);
    if (!tx_buffer.empty()) {
        CANMessage source;
        if (tx_buffer.pop(&source)) {
            long id = (long)(source.vesc_id | (source.cmd_id << 8));
            can_tx(id, source.data, source.length);
        } else {
            report_error("Error dequeing tx can message");
        }
    }
    while (!rx_buffer.empty()) {
        CANMessage item;
        if (rx_buffer.pop(&item)) {
            parse_can_message(&item);
        } else {
            report_error("Error dequeing rx can message");
        }
    }

    can_flags &= ~CAN_FLAG_HANDLER_QUEUED;
    if (can_flags & CAN_FLAG_HANDLER_WAITING) {
        can_flags &= ~CAN_FLAG_HANDLER_WAITING;
        report_error("Error can rx dropped");
    }
}
...
void motor_driver_init(Scheduler* sched) {
    ...
    // attachInterrupt(CAN_INT_PIN, handle_can_rx_isr, FALLING);
    if (sched) {
        sched->register_task("motor_driver", SCHED_MICROSECONDS(CAN_UPDATE_MS),
                             can_update_cb, TASK_FLAG_ENABLED);
    }
    EXIT;
}
